...
import { IconHatColor, IconHatGray, IconLocationColor, IconLocationGray, IconSearch } from "@/public/svgs/search";
...
const SearchPageContent = ({ homeUniversitySlug }: SearchPageContentProps) => {
  ...
  const handleTextSearch = () => {
    if (searchText.trim()) {
      router.push(`/university/${homeUniversitySlug}?searchText=${encodeURIComponent(searchText.trim())}`);
    }
  };
  ...
  return (
    <>
      ...
      <div className="relative mb-6">
        <div className="flex items-center gap-2 rounded-lg border border-k-100 bg-k-50 px-4 py-3">
          <IconSearch className="h-5 w-5 text-k-400" />
          <input
            type="text"
            value={searchText}
