...
export const getUniversityDetailForSsg = async (universityInfoForApplyId: number): Promise<University> => {
  const result = await serverFetch<University>(`/univ-apply-infos/${universityInfoForApplyId}`);
  return assertUniversitySsgResponse(result, `get university detail ${universityInfoForApplyId}`);
};
