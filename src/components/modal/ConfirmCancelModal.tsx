...
const ConfirmCancelModal = ({ isOpen, handleCancel, handleConfirm, title, content }: ConfirmCancelModalProps) => {
  ...
  return (
    <ModalBase isOpen={isOpen} onClose={handleCancel}>
      <div className="w-72 rounded bg-white px-5 py-[22px]">
        ...
        <div className="mt-5 flex gap-1.5">
          <button
            className="flex h-11 w-full cursor-pointer items-center justify-center rounded-lg border border-[#c2c2c2] bg-white px-1.5 py-2"
            onClick={handleCancel}
          >
            <span className="text-center font-serif text-base font-normal leading-4 text-[#3c3c3c]">취소</span>
          </button>
          <button
            className="flex h-11 w-full cursor-pointer items-center justify-center rounded-lg bg-primary-2 px-1.5 py-2"
