...
export function parseBrunoFile(filePath: string): ParsedBrunoFile {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (trimmed === "meta {") {
      ...
      blockContent = [];
      continue;
    } else if (trimmed.match(/^(get|post|put|patch|delete|head|options|trace|connect)\s*\{/i)) {
      // HTTP 메서드 블록 형식: put { url: ... }
      const match = trimmed.match(/^(get|post|put|patch|delete|head|options|trace|connect)\s*\{/i);
      if (match) {
...
export function extractJsonFromDocs(docs: string): any {
  try {
    ...
    let match: RegExpExecArray | null = statusCodePattern.exec(docs);
    let json200 = null;
    ...
    while (match !== null) {
      ...
      match = statusCodePattern.exec(docs);
    }
