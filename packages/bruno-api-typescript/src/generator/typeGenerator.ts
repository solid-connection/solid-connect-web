 ...
 */
export function generateTypeScriptInterface(json: any, interfaceName: string): TypeDefinition[] {
  ...
  if (Array.isArray(json)) {
    ...
    if (json.length === 0) {
      definitions.push({ name: interfaceName, content: `export type ${interfaceName} = unknown[];` });
      return definitions;
    }
    ...
    for (const item of json) {
      ...
      if (typeof item === "object" && !Array.isArray(item)) {
        itemTypes.add(`${interfaceName}Item`);
        continue;
      }

      itemTypes.add(getPropertyType(item, `${interfaceName}Item`, interfaceName));
    }

    const sortedItemTypes = Array.from(itemTypes).sort();
    const arrayType = sortedItemTypes.length === 1 ? `${sortedItemTypes[0]}[]` : `(${sortedItemTypes.join(" | ")})[]`;

    definitions.push({ name: interfaceName, content: `export type ${interfaceName} = ${arrayType};` });
    return definitions;
  }
  ...
  if (json === null || json === undefined || typeof json !== "object") {
    definitions.push({
      name: interfaceName,
      content: `export type ${interfaceName} = ${getPropertyType(json, interfaceName)};`,
    });
    return definitions;
  }
...
function extractNestedTypes(
  ...
  if (Array.isArray(value) && value.length > 0) {
    ...
    if (typeof itemType === "object" && !Array.isArray(itemType) && itemType !== null) {
      ...
      const itemTypeName = parentTypeName ? `${parentTypeName}${typeName}Item` : `${typeName}Item`;
      const properties: string[] = [];
      ...
      for (const [key, typeSet] of fieldTypes) {
        ...
        const propType = types.length === 1 ? types[0] : types.join(" | ");
    ...
    if (!isRoot && parentTypeName && typeName) {
      ...
      for (const [key, val] of Object.entries(value)) {
        const childParentTypeName = isRoot
          ? parentTypeName
          : parentTypeName
